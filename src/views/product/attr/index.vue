        ...
        <el-card style="margin:10px 0px">
            ...
            <div v-show="scene == 0">
                ...
                <el-button @click="addAttr" type="primary" size="default" icon="Plus"
                    :disabled="categoryStore.c3Id ? false : true" v-btn="'btn.Attr.add'">添加属性</el-button>
                <!-- border:竖直方向的边框 prop:用来对应对象中的键名即可填入数据 -->
                    ...
                    <el-table-column label="操作" width="120px">
                        ...
                        <template #="{ row }">
                            ...
                            <el-popconfirm :title="`你确定删除${row.attrName}?`" width="200px" @confirm="deleteAttr(row.id)">
                                ...
                                <template #reference>
                                    <!-- 删除属性按钮 -->
                                    <el-button type="primary" size="small" icon="Delete" v-btn="'btn.Attr.remove'"></el-button>
                                </template>
                            </el-popconfirm>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="属性值名称">
                        ...
                        <template #="{ row, $index }">
                            <!-- :ref="(vc: any) => inputArr[$index] = vc":
                                ref传递了一个函数作为引用。当组件被创建时,这个函数会被调用,并将组件实例vc作为参数。
                                这个函数将组件实例存储在 inputArr[$index] 中,其中 $index 是当前迭代的索引。
                                这样做的目的是为了在父组件中存储对每个输入组件的引用,以便以后可以手动访问或操作这些输入组件。 -->
...
const updateAttr = (row: Attr) => {
    //切换为添加与修改属性的结构
    scene.value = 1;
    //JSON.stringify将JS对象转换成JSON字符串
    //JSON.parse将JSON字符串解析成JS对象
...
const toLook = (row: AttrValue, $index: number) => {
    ...
    //相应的属性值对象flag:变为false,展示div,el-input是否展示
    row.flag = false;
}
