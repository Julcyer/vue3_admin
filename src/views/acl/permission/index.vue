        ...
        <el-table-column label="操作" align="center">
            ...
            <template #="{ row, $index }">
                <el-button type="primary" @click="addPermisstion(row)" size="small"
                    :disabled="row.level == 4 ? true : false" :title="row.level === 3 ? '添加功能' : '添加菜单'" icon="Plus"
                    v-btn="'btn.Permission.add'">
                </el-button>
                <el-button type="primary" @click="updatePermisstion(row)" size="small"
                    :disabled="row.level == 1 ? true : false" v-if="row.level != 1" title="编辑" icon="Edit"
                    v-btn="'btn.Permission.update'"></el-button>
                <el-popconfirm :title="`你确定要删除${row.name}?`" width="260px" @confirm="removeMenu(row.id)">
                    <template #reference>
                        <el-button type="primary" size="small" :disabled="row.level == 1 ? true : false"
                            v-if="row.level != 1" icon="Delete" title="删除"
                            v-btn="'btn.Permission.remove'"></el-button>
                    </template>
                </el-popconfirm>
            </template>
        </el-table-column>
