<template>
    ...
    <el-card style="margin: 10px 0px;">
        <el-button type="primary" size="default" icon="Plus" @click="addRole" v-btn="'btn.Role.add'">添加职位</el-button>
        ...
        事件 @current-change: current-page改变时触发 @size-change: page-size改变时触发 -->
    ...
    <el-drawer v-model="drawer">
        ...
        <template #default>
            <!-- 树形控件 show-checkbox:节点是否可被选择 default-expand-all:默认展开所有节点 
                default-checked-keys:默认勾选的节点key数组 props:配置选项-->
...
//树形控件的测试数据
// props配置选项可能值
// children:指定子树为节点对象的某个属性值,label:指定节点标签为节点对象的某个属性值,
// disabled:指定节点选择框是否禁用为节点对象的某个属性值,isLeaf:指定节点是否为叶子节点,仅在指定了lazy属性的情况下生效,class:自定义节点类名
const defaultProps = {
    children: 'children',
    label: 'name',
}
...
const handler = async () => {
    ...
    //选中节点的ID, getCheckedKeys(),返回节点复选框被选中的key值数组(show-checkbox为true)
    let arr = tree.value.getCheckedKeys();
    //半选的ID, getHalfCheckedKeys(),返回被半选中的节点的key值数组(父节点的一部分子节点被选中)
    let arr1 = tree.value.getHalfCheckedKeys();
    //concat,JS数组方法,合并两个或多个数组
